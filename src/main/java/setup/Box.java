...
public class Box implements Iterable<Ball> {
    ...
    public boolean add(Ball b) {
        if(ballContainer.getVolume()+b.getVolume()<=boxMaxVolume && !ballContainer.contains(b)){
            ballContainer.add(b);
            return true;
        }else{
            return false;
    ...
    public Iterator<Ball> getBallsFromSmallest() {
        ...
        BallList.sort(new ballIterator());
        return BallList.iterator();
    }
    public class ballIterator implements Comparator<Ball>{
        @Override
